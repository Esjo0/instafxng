...
class Signal_Management
{
    ...
    /*
     * get the symbol using symbol_id
     */
    public function get_currency_pair($symbol_id)
    {
        global $db_handle;
        $query = "SELECT SS.symbol FROM signal_daily AS SD INNER JOIN signal_symbol AS SS ON SD.symbol_id = SS.symbol_id WHERE SS.symbol_id = $symbol_id LIMIT 1 ";
        return $db_handle->fetchAssoc($db_handle->runQuery($query))[0]['symbol'];
    }


    /*
     * get the symbol using signal_id
     */
    public function get_currency_pair_signal_id($signal_id)
    {
        global $db_handle;
        $query = "SELECT SS.symbol
FROM signal_daily AS SD INNER JOIN signal_symbol AS SS ON SD.symbol_id = SS.symbol_id WHERE SD.signal_id = '$signal_id'
LIMIT 1";
        return $db_handle->fetchAssoc($db_handle->runQuery($query))[0]['symbol'];
    }
...
KEYNOTE;
    ...
    public function update_signal_schedule($id, $symbol, $price, $take_profit, $stop_loss, $signal_time, $signal_date, $comment='', $trend, $type, $admin)
    {
        ...
        if ($result) {
            ...
            $currency_pair = $this->get_currency_pair($symbol);
            $this->send_push_notification('Instafxng Notification', 'New '.$currency_pair.' Signal Has been Posted.!!! Click to view '.$comment, 'https://instafxng.com/signal_schedules.php');
        }
        return $result;
    }
...
ORDER BY SD.signal_id DESC ";
    ...
    public function trigger_signal_schedule($signal_id, $trigger_status, $entry_price, $entry_time, $exit_time, $pips, $exit_type, $exit_price, $highest_pips_time, $lowest_pips_time, $highest_pips, $lowest_pips)
    {
        ...
        if ($result) {
            $signal_array = $this->get_scheduled_signals(date('Y-m-d'));
            $this->update_signal_daily_FILE($signal_array);
            $currency_pair = $this->get_currency_pair_signal_id($signal_id);
            $this->send_push_notification('Instafxng Notification', $currency_pair.' Has an Update.!!! Signals Status :'.$this->UI_get_signal_trigger_status_msg($trigger_status), 'https://instafxng.com/signal_schedules.php');
    ...
    public function new_signal_schedule($symbol_id, $order_type, $price, $take_profit, $stop_loss, $trigger_date, $trigger_time, $trend, $note = '', $admin_code, $market_price, $status)
    {
        ...
        if ($result) {
            ...
            $currency_pair = $this->get_currency_pair($symbol_id);
            $signal_array = $this->get_scheduled_signals(date('Y-m-d'));
            $this->update_signal_daily_FILE($signal_array);
            $this->send_push_notification('Instafxng Notification', 'New '.$currency_pair.' Signal Has been Posted.!!! Click to view '.$note, 'https://instafxng.com/signal_schedules.php');
...
// Send Notificaton for all quarter of the pips gained.
    ...
    public function send_push_notification($title, $body, $link)
    {
        global $db_handle;
        global $system_object;

        $query = "SELECT token FROM push_notification_token";
        $result = $db_handle->runQuery($query);
        $all_tokens = $db_handle->fetchAssoc($result);
        foreach($all_tokens AS $row){
            ...
            $headers = array();
            $headers[] = 'Authorization: key=AAAAm_6qmw4:APA91bFO4_UBw0gEILzDa3ux_pX-6uSh5YOMIN4YlhDjtQVukztsjef2nyxycKjNxGknPelJO-sej--JkUu2v8SEAx_A6_qy3NMR0dvfFakSOk5DifO2Fsn7T3Aw70Scff6pO4mHwpcrpdsR6kvdq2XiMxv8XV4Vhw';
//            $headers[] = 'Authorization: key=AAAAFCNHAE0:APA91bE8SAt1Qm-tXrFxwgDOCFV-3FknheMSjHFqG08Ren13UNGYEvar0BXeMGqACkxhldjgmouM2Q_l5VIsYjzxd8e-M-yOCw9K8pAVmuc02f5ybnUhcxMmf9qOmfelLMZmKVSGyzs0';
