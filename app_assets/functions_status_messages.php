...
function client_group_query($client_group, $campaign_type) {
    ...
    if($campaign_type == 1) {
        ...
        switch ($client_group) {
            ...
            case '53': $query = "SELECT u.first_name, u.email, u.phone, u.status, u.user_code, ud.order_complete_time FROM user_deposit AS ud INNER JOIN user_ifxaccount AS ui ON ud.ifxaccount_id = ui.ifxaccount_id INNER JOIN user AS u ON ui.user_code = u.user_code WHERE ud.status = '8' AND STR_TO_DATE(ud.order_complete_time, '%Y-%m-%d') BETWEEN '$from_date' AND '$to_date' GROUP BY u.user_code "; break;
            case '54': $query = "SELECT u.first_name, u.email, u.phone, u.status, u.user_code, uw.created FROM user_withdrawal AS uw INNER JOIN user_ifxaccount AS ui ON uw.ifxaccount_id = ui.ifxaccount_id INNER JOIN user AS u ON ui.user_code = u.user_code WHERE uw.status = '7' AND STR_TO_DATE(uw.created, '%Y-%m-%d') BETWEEN '$from_date' AND '$to_date'  GROUP BY u.user_code "; break;
