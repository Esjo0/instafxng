...
function client_group_query($client_group, $campaign_type) {
    ...
    if($campaign_type == 1) {
        ...
        switch ($client_group) {
            ...
            case '52': $query = "SELECT user.user_code, user.first_name, user.email, campaign_leads.phone FROM campaign_leads, user WHERE(STR_TO_DATE(campaign_leads.created, '%Y-%m-%d') BETWEEN '2018-04-01' AND '2018-04-30') campaign_leads.email = user.email AND campaign_leads.interest = '2' GROUP BY user.email"; break;
        ...
        switch ($client_group) {
            ...
            case '50': $query = "SELECT u.user_code, u.first_name, u.email, u.phone FROM user AS u LEFT JOIN user_ifxaccount AS ui ON u.user_code = ui.user_code LEFT JOIN user_deposit AS ud ON ui.ifxaccount_id = ud.ifxaccount_id WHERE (STR_TO_DATE(u.created, '%Y-%m-%d') BETWEEN '$from_date' AND '$to_date') AND u.user_code = ui.user_code AND ui.ifxaccount_id = ud.ifxaccount_id AND ud.real_dollar_equivalent < 50.00 AND u.campaign_subscribe = '1' GROUP BY u.user_code "; break;
            case '51': $query = "SELECT u.first_name, u.email, u.phone, u.status, u.user_code, ud.order_complete_time FROM user_deposit AS ud INNER JOIN user_ifxaccount AS ui ON ud.ifxaccount_id = ui.ifxaccount_id INNER JOIN user AS u ON ui.user_code = u.user_code WHERE ud.status = '8' AND u.user_code NOT IN (SELECT u.user_code FROM user_deposit AS ud INNER JOIN user_ifxaccount AS ui ON ud.ifxaccount_id = ui.ifxaccount_id INNER JOIN user AS u ON ui.user_code = u.user_code WHERE ud.status = '8' AND STR_TO_DATE(ud.order_complete_time, '%Y-%m-%d') BETWEEN '2018-03-22' AND '$date_today') GROUP BY u.phone "; break;
            case '52': $query = "SELECT user.user_code, user.first_name, user.email, campaign_leads.phone FROM campaign_leads, user WHERE(STR_TO_DATE(campaign_leads.created, '%Y-%m-%d') BETWEEN '2018-04-01' AND '2018-04-30') campaign_leads.email = user.email AND campaign_leads.interest = '2' GROUP BY user.user_code "; break;
            default: $query = false; break;
        }
