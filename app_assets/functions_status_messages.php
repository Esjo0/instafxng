...
function client_group_campaign_category($status)
{
    switch ($status) {
        ...
        case '54':
            ...
            break;
        case '55':
            $message = "All inactive trading clients after January 1 2017.";
            break;
        case '56':
        ...
        case '69':
            ...
            break;
        case '70':
            $message = "All Clients Not on board";
            break;
        case '71':
            $message = "Funded ILPR account Not on board";
            break;
        case '72':
            $message = "Funded NON-ILPR account Not on board";
            break;
        case '73':
            $message = "All ILPR accounts Not on board";
            break;
        case '74':
    ...
    if ($campaign_type == 1) {
        ...
        switch ($client_group) {
            ...
            case '41':
                ...
                break;
            case '42':
                $query = "SELECT u.user_code, u.first_name, u.email, u.phone FROM trading_commission AS td INNER JOIN user_ifxaccount AS ui ON td.ifx_acct_no = ui.ifx_acct_no INNER JOIN user AS u ON ui.user_code = u.user_code WHERE u.user_code NOT IN (SELECT u.user_code FROM trading_commission AS td INNER JOIN user_ifxaccount AS ui ON td.ifx_acct_no = ui.ifx_acct_no INNER JOIN user AS u ON ui.user_code = u.user_code WHERE td.date_earned > DATE_SUB(NOW(), INTERVAL 3 MONTH)) GROUP BY u.user_code ";
                break;
            case '43':
                $query = "SELECT u.user_code, u.first_name, u.email, u.phone FROM trading_commission AS td INNER JOIN user_ifxaccount AS ui ON td.ifx_acct_no = ui.ifx_acct_no INNER JOIN user AS u ON ui.user_code = u.user_code WHERE u.user_code NOT IN (SELECT u.user_code FROM trading_commission AS td INNER JOIN user_ifxaccount AS ui ON td.ifx_acct_no = ui.ifx_acct_no INNER JOIN user AS u ON ui.user_code = u.user_code WHERE td.date_earned > DATE_SUB(NOW(), INTERVAL 6 MONTH)) GROUP BY u.user_code ";
                break;
            ...
            case '54':
                ...
                break;
            case '55':
                $query = "SELECT u.user_code, u.first_name, u.email, u.phone, MAX(td.date_earned) AS last_trade_date FROM trading_commission AS td INNER JOIN user_ifxaccount AS ui ON td.ifx_acct_no = ui.ifx_acct_no INNER JOIN user AS u ON ui.user_code = u.user_code WHERE u.user_code NOT IN ( SELECT u.user_code FROM trading_commission AS td INNER JOIN user_ifxaccount AS ui ON td.ifx_acct_no = ui.ifx_acct_no INNER JOIN user AS u ON ui.user_code = u.user_code WHERE STR_TO_DATE(td.date_earned, '%Y-%m-%d') BETWEEN '2017-01-01' AND NOW() ) GROUP BY u.user_code ORDER BY last_trade_date DESC ";
                break;
            case '56':
            ...
            case '59':
                ...
                break;
            case '60':
                $query = "SELECT SUM(td.commission) AS total_commission, u.user_code, u.first_name, u.email, u.phone FROM trading_commission AS td INNER JOIN user_ifxaccount AS ui ON ui.ifx_acct_no = td.ifx_acct_no INNER JOIN user AS u ON ui.user_code = u.user_code INNER JOIN account_officers AS ao ON u.attendant = ao.account_officers_id INNER JOIN admin AS a ON ao.admin_code = a.admin_code WHERE date_earned BETWEEN '2017-12-01' AND '2018-11-30' GROUP BY u.user_code HAVING total_commission >= 1000 ";
                break;
            case '61':
