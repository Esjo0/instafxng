...
$query = "SELECT SUM(ud.real_dollar_equivalent) AS sum_deposit, u.user_code
    ...
    WHERE ud.status = '8' AND (STR_TO_DATE(ud.order_complete_time, '%Y-%m-%d') BETWEEN '$from_date' AND '$to_date') AND (STR_TO_DATE(td.date_earned, '%Y-%m-%d') BETWEEN '$from_date' AND '$to_date')";
$result = $db_handle->runQuery($query);
$data_analysis = $db_handle->fetchAssoc($result);
