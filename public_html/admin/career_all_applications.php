...
if(isset($_POST['search_text']) && strlen($_POST['search_text']) > 3) {
    ...
    $query = "SELECT CONCAT(cub.last_name, SPACE(1), cub.first_name) AS full_name, cua.career_user_application_id, cj.title, cua.created, cua.status
        ...
        WHERE (cub.email_address LIKE '%$search_text%' OR cub.last_name LIKE '%$search_text%' OR cub.first_name LIKE '%$search_text%' OR cub.other_names LIKE '%$search_text%' OR cub.email_address LIKE '%$search_text%' OR cub.phone_number LIKE '%$search_text%' OR cub.created LIKE '$search_text%') ORDER BY cua.created DESC ";
} else {
    $query = "SELECT CONCAT(cub.last_name, SPACE(1), cub.first_name) AS full_name, cua.career_user_application_id, cj.title, cua.created, cua.status
        FROM career_user_application AS cua
        INNER JOIN career_user_biodata AS cub ON cua.cu_user_code = cub.cu_user_code
        INNER JOIN career_jobs AS cj ON cua.job_code = cj.job_code ORDER BY cua.created DESC ";
}
