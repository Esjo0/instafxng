...
WHERE user_code IN (SELECT user_code FROM user AS U WHERE U.academy_signup IS NOT NULL)";
 ...
 CONCAT(a.last_name, SPACE(1), a.first_name) AS account_officer_full_name
            ...
            WHERE (uv.phone_status = '2') AND (uc.doc_status != '111')
            AND (ub.status != '2' OR ub.bank_acct_no IS NULL) AND u.email IN(SELECT email FROM unverified_campaign_mail_log)
    ...
    case 'level2':
        $query = "SELECT u.user_code, CONCAT(u.last_name, SPACE(1), u.first_name) AS full_name, u.email, u.phone, u.created, CONCAT(a.last_name, SPACE(1), a.first_name) AS account_officer_full_name
            ...
            INNER JOIN admin AS a ON ao.admin_code = a.admin_code
            LEFT JOIN user_bank AS ub ON u.user_code = ub.user_code
            WHERE (uc.doc_status = '111') AND (ub.status != '2' OR ub.bank_acct_no IS NULL)
    ...
    case 'deposit':
        $query = "SELECT u.user_code, CONCAT(u.last_name, SPACE(1), u.first_name) AS full_name, u.email, u.phone, u.created, CONCAT(a.last_name, SPACE(1), a.first_name) AS account_officer_full_name
            ...
            INNER JOIN account_officers AS ao ON u.attendant = ao.account_officers_id
            INNER JOIN admin AS a ON ao.admin_code = a.admin_code
            WHERE u.email IN(SELECT email FROM unverified_campaign_mail_log)
            GROUP BY u.email ORDER BY u.created DESC";
        $filter_category = "Unverfied Campaign - Funded";
        $display_msg = "Unverified clients who have funded their accounts after receiving the scheduled mail campaign";
        break;
    ...
    case 'trading':
        $query = "SELECT u.user_code, CONCAT(u.last_name, SPACE(1), u.first_name) AS full_name, u.email, u.phone, u.created, CONCAT(a.last_name, SPACE(1), a.first_name) AS account_officer_full_name
            ...
            WHERE u.email IN(SELECT email FROM unverified_campaign_mail_log)
            GROUP BY u.email ORDER BY u.created DESC";
        $filter_category = "Unverfied Campaign - Trading";
        $display_msg = "Unverified clients who have started trading activity after receiving the scheduled mail campaign";
        break;
...
AND (full_name LIKE '%$search_text%' OR email LIKE '%$search_text%' OR phone LIKE '%$search_text%')";
        ...
        case 'level1':
            ...
            LEFT JOIN user_bank AS ub ON u.user_code = ub.user_code
            LEFT JOIN user_credential AS uc ON u.user_code = uc.user_code
            WHERE (uv.phone_status = '2') AND (uc.doc_status != '111') AND (ub.status != '2' OR ub.bank_acct_no IS NULL)
            AND (ui.ifx_acct_no LIKE '%$search_text%' OR u.email LIKE '%$search_text%'
            OR u.first_name LIKE '%$search_text%' OR u.middle_name LIKE '%$search_text%'
            OR u.last_name LIKE '%$search_text%' OR u.phone LIKE '%$search_text%' OR u.created LIKE '$search_text%')
             AND u.email IN(SELECT email FROM unverified_campaign_mail_log)
             GROUP BY u.email ORDER BY u.created DESC";
            $filter_category = "Unverified to Level1";
            $display_msg = "Unverified clients who moved to the Level 1 Through the scheduled mail campaign.";
        ...
        case 'level3':
            ...
            WHERE (ub.is_active = '1' AND ub.status = '2') AND (ui.ifx_acct_no LIKE '%$search_text%'
            OR u.email LIKE '%$search_text%' OR u.first_name LIKE '%$search_text%' OR u.middle_name LIKE '%$search_text%'
            OR u.last_name LIKE '%$search_text%' OR u.phone LIKE '%$search_text%' OR u.created LIKE '$search_text%')
            AND u.email IN(SELECT email FROM unverified_campaign_mail_log)
            GROUP BY u.email ORDER BY u.created DESC";
            $filter_category = "Unverified to Level3";
        ...
        case 'deposit':
            ...
            INNER JOIN account_officers AS ao ON u.attendant = ao.account_officers_id
            INNER JOIN admin AS a ON ao.admin_code = a.admin_code
            WHERE (ui.ifx_acct_no LIKE '%$search_text%' OR u.email LIKE '%$search_text%'
            OR u.first_name LIKE '%$search_text%' OR u.middle_name LIKE '%$search_text%'
            OR u.last_name LIKE '%$search_text%' OR u.phone LIKE '%$search_text%'
            OR u.created LIKE '$search_text%')
            AND u.email IN(SELECT email FROM unverified_campaign_mail_log)
            GROUP BY u.email ORDER BY u.created DESC";
            $filter_category = "Unverfied Campaign - Funded";
            $display_msg = "Unverified clients who have funded their accounts after recieving the scheduled mail campaign";
            break;
                                            ...
                                            <div class="input-group input-group-sm">
                                                ...
                                                <div class="input-group-btn input-group-select">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                        ...
                                                        <li><a onclick="filter('deposit', 'Unverfied Campaign - Funded')" href="javascript:void(0);">Unverfied Campaign - Funded</a></li>
                                                        <li><a onclick="filter('trading', 'Unverfied Campaign - Trading')" href="javascript:void(0);">Unverfied Campaign - Trading Clients</a></li>
                                                    </ul>
