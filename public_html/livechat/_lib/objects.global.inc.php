...
class Post extends BaseObject
{
    ...
    static function ProcessPostForExternal($_operator,$_postId,$rec,$recgroup,$post,$time,$_group,$_translation,$_translationISO)
    {
        if(STATS_ACTIVE)
...
class Ticket extends Action
{
        ...
        if($_oldGroup != $_newGroup)
        {
            ...
            if($group->SendTicketNotifier)
            {
                ...
                if(!empty($mailbox))
                    ...
                    {
                        $body = $LZLANG["client_group_new_ticket_notify"];
                        $body = str_replace("<!--ticket_id-->",$this->Id,$body);
                        $body = str_replace("<!--operator_name-->",$operator->Fullname,$body);
                        $body = str_replace("<!--group_name-->",$group->Id,$body);
                        $body = str_replace("<!--client_url-->",LIVEZILLA_URL . "mobile/",$body);
                        ...
                        Communication::SendEmail($mailbox, str_replace(array(",,", ";"), ",", $operator->Email), "", "", $mailbox->Email, $body, "", $subject, false, null, "", "");
                    }
            }
        }
    }
    ...
    static function GetSearchQuery($_sFor)
    {
        ...
        if($ss[13]=="1")$query .= " OR LOWER(`tcom`.`comment`) LIKE '%".$_sFor."%'";
