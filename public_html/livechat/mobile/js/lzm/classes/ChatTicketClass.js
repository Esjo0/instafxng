...
ChatTicketClass.prototype.showMessageReply = function(ticket, messageNo, selectedGroup, menuEntry) {
    ...
    replyString += '</div></fieldset>' +
        '<fieldset class="lzm-fieldset"><legend>' + t('Introduction Phrase') + '</legend>' +
        '<div class="tr-salutation-fields' + disabledString2 + '">';
    ...
    replyString += '<div id="ticket-reply-input-buttons" class="bottom-space" style="padding:5px 0;">';
        replyString += lzm_inputControls.createButton('ticket-reply-input-load', '', '', t('Load'), '<i class="fa fa-folder-open-o"></i>', 'lr', {'margin-left': '0px'},'',-1,'e') +
        lzm_inputControls.createButton('ticket-reply-input-save', 'ui-disabled', '', tid('save'), '<i class="fa fa-save"></i>', 'lr',  {'margin-left': '-1px'},'',-1,'e') +
        lzm_inputControls.createButton('ticket-reply-input-saveas', '', '', tid('ticket_saveas'), '<i class="fa fa-plus"></i>', 'lr', {'margin-left': '-1px'},'',-1,'e') +
        lzm_inputControls.createButton('ticket-reply-input-clear', '', '', tid('ticket_clear'), '<i class="fa fa-remove"></i>', 'lr',{'margin-left': '-1px'},'',-1,'e') +
        lzm_inputControls.createButton('ticket-reply-reply-inline', '', '', tid('reply_inline'), '<i class="fa fa-terminal"></i>', 'lr',{'margin-left': '-1px'},'',-1,'e') +
        lzm_inputControls.createButton('ticket-reply-show-question', '', '', tid('show_question'), '<i class="fa fa-question"></i>', 'lr',{'margin-left': '-1px'},'',-1,'e') +
    ...
    replyString += '</fieldset><fieldset class="lzm-fieldset"><legend>' + t('Closing Phrase') + '</legend>';
    ...
    replyString += '<span id="tr-close-placeholder"' + disabledString + '></span>' +
        ...
        '</fieldset>';
    replyString += '<fieldset class="lzm-fieldset"><legend>' + tid('signature') + '</legend><div id="message-signature-container" class="' + disabledString2 + '" style="margin: 0px; width:100%;">' +
    ...
    lzm_inputControls.createInputMenu('tr-intro-placeholder', 'tr-intro', '', 0, t('Introduction Phrase'), salutationFields['introduction phrase'][1][0][0], salutationFields['introduction phrase'][1], 'reply-placeholder-content-0', 8);
    lzm_inputControls.createInputMenu('tr-close-placeholder', 'tr-close', '', 0, t('Closing Phrase'), salutationFields['closing phrase'][1][0][0], salutationFields['closing phrase'][1], 'reply-placeholder-content-0', 22);
...
ChatTicketClass.prototype.ShowMessageReplyPreview = function(ticket, messageNo, message, signature, comment, attachments, salutation, group, menuEntry) {
    ...
    /*else*/if(d(ticket.messages) && ticket.messages.length && rootMessage != null && rootMessage.s.length)
    ...
    $('#ticket-reply-send').click(function() {
        ...
        try
        {
            // check if email field is masked
...
ChatTicketClass.prototype.ShowEmailLists = function() {
    ...
    var footerString = lzm_inputControls.createButton('save-email-list', '','', t('Ok'), '', 'lr',    {'margin-left': '6px'}, '' ,30, 'd') +
        ...
        lzm_inputControls.createButton('delete-email', '','', tidc('delete',' (Del)'), '<i class="fa fa-remove"></i>', 'lr', {'margin-left': '6px', 'margin-top': '-4px'} , '' ,30, 'e') +
...
ChatTicketClass.UpdateTicketQuickSearch = function(){
    var cursorPosition = $('#ticket-reply-input').prop("selectionStart");
    ChatTicketClass.ComposerLastCursorPosition = cursorPosition;
    var word,text = $('#ticket-reply-input').val().toString();

    word = lzm_commonTools.GetLastWordAt(text,cursorPosition);
