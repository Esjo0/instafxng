...
if(isset($get_params['trans_id'])) {
    ...
    if($db_handle->numOfRows($result) > 0) {
        ...
        // Convert amount from kobo back to naira before storing.
        $paystack_response_amount = $paystack_response['data']['amount'] / 100;

        // Log Deposit Meta Data Returned from payment gateway
        $log_deposit_meta = $client_operation->log_paystack_meta($user_deposit_id, $paystack_response['data']['status'], $paystack_response['data']['gateway_response'], $paystack_response_amount, $paystack_response['data']['currency'], $paystack_response['data']['channel']);
...
MAIL;
            ...
            $gateway_response = $paystack_response['data']['gateway_response'];

            // Update the transaction
            $query = "UPDATE user_deposit SET client_naira_notified = '$client_naira_notified', client_pay_date = NOW(), client_pay_method = '10', client_notified_date = NOW(), status = '9', client_comment = '$gateway_response' WHERE trans_id = '$trans_id' LIMIT 1";
            $db_handle->runQuery($query);
